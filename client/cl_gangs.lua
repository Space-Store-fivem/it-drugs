...
function refreshGangZones()
    ...
    for zoneId, zone in pairs(gangZones) do
        if zone.polygon_points and #zone.polygon_points >= 3 then
            ...
            --[=[ 
            lib.zones.poly replaced to prevent ox_lib error
    ...
    while true do
        ...
        if foundZone ~= currentGangZoneId then
            ...
            if currentGangZoneId then
                ...
                local lastZone = gangZones[currentGangZoneId]
                local lastLabel = lastZone and (lastZone.label or "Território") or "Território"
            ...
            if foundZone then
                ...
                if owner ~= 'Ninguém' then
                    if playerGang and playerGang.name == owner then
                        status = 'friendly'
                    else
                        status = 'hostile'
                    end
